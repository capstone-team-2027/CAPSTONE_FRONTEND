...
export default function ReceptionCreateServiceOrder() {
  ...
  // Ở tab "Khách lần đầu đến": SĐT vừa gõ có thể đã thuộc về 1 khách có sẵn trong hệ thống —
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetchPrivate(SEARCH_API_ENDPOINTS.CUSTOMER_INFO_BY_PHONE, 'POST', { phone: searchTerm });
        const foundCustomer = res?.data?.customer;
        const matchesPhone = foundCustomer && String(foundCustomer.phone).replace(/\D/g, '').replace(/^84/, '') === searchTerm;
        setDuplicateCustomer(
          matchesPhone
            ? { type: 'customer', id: foundCustomer.id, name: foundCustomer.customer_name, phone: foundCustomer.phone, vehicles: foundCustomer.vehicles || [] }
            : null,
        );
      } catch (err) {
