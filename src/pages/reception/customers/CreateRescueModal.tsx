...
export const CreateRescueModal: React.FC<CreateRescueModalProps> = ({
  ...
  const handleSearchAddress = async () => {
    ...
    try {
      const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchAddressQuery)}&limit=5&countrycodes=vn`;
      ...
      showToast("Lỗi khi kết nối dịch vụ bản đồ", "warning");
    } finally {
            ...
            <div className="relative">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#00285E]/10 focus:border-[#00285E] transition-all"
                />
                <button
                  type="button"
                  onClick={handleSearchAddress}
                  disabled={searchingAddress}
                  className="px-4 py-2.5 bg-[#00285E] hover:bg-[#003a8a] text-white text-sm font-semibold rounded-lg transition-colors shrink-0 flex items-center gap-1.5"
                >
                  <Search size={16} />
              ...
              {searchResults.length > 0 && (
                <div className="absolute left-0 right-0 z-[200] mt-1 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
