...
export default function TechnicianRescuePage() {
  ...
  useEffect(() => {
    if (socket && rescueTask && rescueTask.status === 'EN_ROUTE' && technicianLocation) {
      const customerId = rescueTask.customer_id;
      if (customerId) {
        socket.emit('update-rescue-location', {
          customerId: customerId,
          latitude: technicianLocation[0],
          longitude: technicianLocation[1]
        });
      }
    }
  }, [socket, rescueTask, technicianLocation]);
